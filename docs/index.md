...
- `auth_method` (String) Authentication method (e.g., identity, identity_service_user). Resolved from environment variable IDSEC_AUTH_METHOD.
- `cache_authentication` (Boolean) Cache authentication for the provider. Resolves from environment variable IDSEC_CACHE_AUTHENTICATION.
- `secret` (String, Sensitive) Secret for identity authentication. Resolved from environment variable IDSEC_SECRET.
- `service_authorized_app` (String) Authorized application for identity service user authentication. Resolved from environment variable IDSEC_SERVICE_AUTHORIZED_APP.
- `service_token` (String, Sensitive) Service token for identity service user authentication. Resolved from environment variable IDSEC_SERVICE_TOKEN.
- `service_user` (String) Service user for identity service user authentication. Resolved from environment variable IDSEC_SERVICE_USER.
- `subdomain` (String) Tenant Subdomain for identity authentication. Resolved from environment variable IDSEC_SUBDOMAIN.
- `username` (String) Username for identity authentication. Resolved from environment variable IDSEC_USERNAME.
