---
page_title: "Create access control policy for AWS IAM Identity Center"
description: |-
...
3. Create a policy for AWS IAM Identity Center access
...
resource "idsec_policy_cloud_access" "example_aws_iam_idc_policy" {
  ...
  metadata = {
    ...
    policy_entitlement = {
      ...
      location_type   = "AWS" #var.location_type
  ...
  targets = {
    aws_organization_targets = [
      {
        role_id      = "arn:aws:sso:::permissionSet/ssoins-55555cf0998b940/ps-48b59e1afd27e74e" #var.role_id
        workspace_id = "123451234000" #var.workspace_id
        org_id       = "1234567891234" #var.org_id
      },
      {
        role_id      = "arn:aws:sso:::permissionSet/ssoins-66666cf0998b940/ps-511c1a3285b339c7"
        workspace_id = "123451234000"
        org_id       = "1234567891234"
      }
    ]
  }
  principals = [
...
variable "policy_tags" {
  description = "Customized tags to help identify the policy and those similar to it - maximum 20 tags per policy. (e.g. ['test_aws_iam_idc'])"
  type        = list(string)
  default     = []
  validation {
...
variable "workspace_id" {
  description = "The ID given to the AWS organization workspace when it was onboarded to Idira. Required."
  type        = string
}
variable "org_id" {
