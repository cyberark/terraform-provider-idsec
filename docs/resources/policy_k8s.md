...
- `aws_account_targets` (Attributes List) AWS IAM K8s cluster target details (see [below for nested schema](#nestedatt--targets--aws_account_targets))
- `aws_idc_targets` (Attributes List) AWS Identity Center K8s cluster target details (see [below for nested schema](#nestedatt--targets--aws_idc_targets))
- `azure_targets` (Attributes List) Azure K8s cluster target details (see [below for nested schema](#nestedatt--targets--azure_targets))
