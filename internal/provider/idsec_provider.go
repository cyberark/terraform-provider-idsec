	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			"auth_method": schema.StringAttribute{
				Optional:    true,
				Description: "Authentication method (e.g., identity, identity_service_user). Resolved from environment variable IDSEC_AUTH_METHOD.",
			...
			"username": schema.StringAttribute{
				Optional:    true,
				Description: "Username for identity authentication. Resolved from environment variable IDSEC_USERNAME.",
			},
			"secret": schema.StringAttribute{
				Optional:    true,
				Description: "Secret for identity authentication. Resolved from environment variable IDSEC_SECRET.",
				Sensitive:   true,
			},
			"service_user": schema.StringAttribute{
				Optional:    true,
				Description: "Service user for identity service user authentication. Resolved from environment variable IDSEC_SERVICE_USER.",
			},
			"service_token": schema.StringAttribute{
				Optional:    true,
				Description: "Service token for identity service user authentication. Resolved from environment variable IDSEC_SERVICE_TOKEN.",
				Sensitive:   true,
			},
			"service_authorized_app": schema.StringAttribute{
				Optional:    true,
				Description: "Authorized application for identity service user authentication. Resolved from environment variable IDSEC_SERVICE_AUTHORIZED_APP.",
			},
			"subdomain": schema.StringAttribute{
				Optional:    true,
				Description: "Tenant Subdomain for identity authentication. Resolved from environment variable IDSEC_SUBDOMAIN.",
			},
			"cache_authentication": schema.BoolAttribute{
				Optional:    true,
				Description: "Cache authentication for the provider. Resolves from environment variable IDSEC_CACHE_AUTHENTICATION.",
			},
		},
	}
}
